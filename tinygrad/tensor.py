...
class Tensor:
  ...
  @staticmethod
  def zeros(*shape, **kwargs): return Tensor(0, **kwargs).reshape([1]*(len(new_shape := argfix(*shape)))).expand(new_shape).contiguous()

  @staticmethod
  def ones(*shape, **kwargs): return Tensor(1, **kwargs).reshape([1]*len(new_shape := argfix(*shape))).expand(new_shape).contiguous()
  ...
  # - Strides other than 1 are allowed:
  ...
  #        - So pad dim_sz with as many zeros (dim_sz -> dim_sz_padded) as needed so that reshape [dim_sz_padded // s, s]
  #          is possible.
  #        - Shrink to do [:, 0] on [dim_sz_padded // s, s]
